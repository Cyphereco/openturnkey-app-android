...
//    }
    ...
    public ECDSASignature createSignature(byte[] contents) throws ECException {
        ...
        Log.d("JJJ", BtcUtils.bytesToHexString(contentsHash));
        //
     ...
     */
    public boolean verifySignature(byte[] contents, String signature) throws ECException {
        boolean isValid = false;
...
//        ECDSASignature sig = new ECDSASignature(signature);
    ...
    public String signMessage(String message) throws ECException {
        ...
        try {
            ...
            try (ByteArrayOutputStream outStream = new ByteArrayOutputStream(message.length()*2)) {
                ...
                Log.d("JJJ", BtcUtils.bytesToHexString(contents));
            }
    ...
    public static boolean verifyMessage(String address, String message, String encodedSignature)
        ...
        try {
            ...
            throw new VarInt.SignatureException("Unable to decode the signature", exc);
        }
        //
        ...
        if (decodedSignature.length < 65)
            throw new VarInt.SignatureException("Signature is too short");
        int headerByte = (int)decodedSignature[0]&0xff;
        if (headerByte < 27 || headerByte > 34)
            throw new VarInt.SignatureException(String.format("Header byte %d is out of range", headerByte));
        ...
        try {
            ...
            if (key == null)
                throw new VarInt.SignatureException("Unable to recover public key from signature");
        } catch (IOException exc) {
            throw new IllegalStateException("Unexpected IOException", exc);
        } catch (IllegalArgumentException exc) {
            throw new VarInt.SignatureException("Signature is not valid");
        }
        //
     ...
     */
    public static ECKey recoverFromSignature(int recID, ECDSASignature sig, BigInteger e, boolean compressed) {
